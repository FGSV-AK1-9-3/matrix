...
- Name: scenic.spec.js >> Tab navigation guard >> can navigate to tab 3 after filling tabs 1 and 2
- Location: scenic.spec.js:291:7
...
Call log:
  - waiting for locator('#veranstaltungsgelaende_flaechenverhaeltnis')
    - locator resolved to <select required="" class="form-select" id="veranstaltungsgelaende_flaechenverhaeltnis">…</select>
          ...
          - generic [ref=e46]:
            ...
            - generic [ref=e50]:
              - generic [ref=e51]:
                ...
                - generic [ref=e52]: "?"
                - text: "*"
              - spinbutton "Personenzahl ? *" [active] [ref=e53]: "500"
        - text: "* * * * * * * * * * * * * * * * *  "
      - generic [ref=e54]:
...
> 72  |       await page.selectOption('#veranstaltungsgelaende_flaechenverhaeltnis', '1.0');
  ...
  151 |   });
  152 | 
  153 |   test('field with value 0 gains is-invalid class', async ({ page }) => {
  154 |     await page.fill('#grunddaten_personenzahl', '0');
  155 |     await page.locator('#grunddaten_personenzahl').dispatchEvent('change');
  156 |     await expect(page.locator('#grunddaten_personenzahl')).toHaveClass(/is-invalid/);
  157 |   });
  158 | 
  159 |   test('Veranstaltungsname field is optional and accepts text', async ({ page }) => {
  160 |     await page.fill('#grunddaten_veranstaltungsname', 'Stadtfest 2025');
  161 |     await expect(page.locator('#grunddaten_veranstaltungsname')).toHaveValue('Stadtfest 2025');
  162 |   });
  163 | 
  164 |   test('progress updates after filling Personenzahl', async ({ page }) => {
  165 |     await page.fill('#grunddaten_personenzahl', '1000');
  166 |     await page.locator('#grunddaten_personenzahl').dispatchEvent('change');
  167 |     const text = await page.locator('#progress_text').textContent();
  168 |     expect(parseInt(text)).toBeGreaterThan(0);
  169 |   });
  170 | });
  171 | 
  172 | // ─────────────────────────────────────────────────────────────────────────────
